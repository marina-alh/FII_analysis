...
#    codigos_da_carteira_b3_acoes_eod = cotacoes.carregar_codigos(['IBRA.sa','FII.sa','BOVA11.sa'])
    codigos_da_carteira_b3_acoes_eod = cotacoes.carregar_codigos(['IBRA.sa'])
#    codigos_da_carteira_b3_acoes_eod = cotacoes.carregar_codigos(['ALPA4.sa', 'ANIM3.sa','B3SA3.sa','BBDC3.sa','BPAN4.sa','BRFS3.sa','CCRO3.sa','CEAB3.sa','CNTO3.sa','COGN3.sa','DMMO3.sa',
#               'EMBR3.sa','ITSA4.sa','ITUB3.sa','ITUB4.sa','LOGG3.sa','MEAL3.sa','MRVE3.sa','MULT3.sa','NTCO3.sa','PRIO3.sa','SQIA3.sa','TIMP3.sa',
#               'TRIS3.sa','VALE3.sa','VIVA3.sa','VIVT4.sa','VLID3.sa','VVAR3.sa','WIZS3.sa','YDUQ3.sa'])
#    codigos_da_carteira_nyse_acoes_eod = cotacoes.carregar_codigos(['NYSE'])
#    codigos_da_carteira_nasdaq_acoes_eod = cotacoes.carregar_codigos(['NASDAQ'])
#    codigos_da_carteira_crypto_eod = cotacoes.carregar_codigos(['CRYPTO'])
#    codigos_da_carteira_currency_eod = cotacoes.carregar_codigos(['CURRENCY'])
#    codigos_da_carteira_global_indices_eod = cotacoes.carregar_codigos(['GLOBAL INDICES'])
...
#    atualizar_cotacoes_yahoo_acoes_eod(codigos_da_carteira_nyse_acoes_eod)
#    atualizar_cotacoes_yahoo_acoes_eod(codigos_da_carteira_nasdaq_acoes_eod)
#    atualizar_cotacoes_yahoo_acoes_eod(codigos_da_carteira_crypto_eod)
#    atualizar_cotacoes_yahoo_acoes_eod(codigos_da_carteira_currency_eod)
#    atualizar_cotacoes_yahoo_acoes_eod(codigos_da_carteira_global_indices_eod)
...
def atualizar_cotacoes_yahoo_acoes_eod(codigos_da_carteira):
    ...
    path = 'D:/marin/Documents/dados/yahoo/IBRA/Yahoo-IBRA.daily/'
    # path = 'D:/marin/Documents/dados/yahoo/FII/Yahoo-FII.daily/'
    os.makedirs(path, exist_ok=True)
    for codigo in codigos_da_carteira:
        filename_csv = path+codigo+'.csv';
        print("[",codigo,"] Buscando cotações...")
        df_cotacoes = pd.DataFrame(columns=('date','open','high','low','close','adj close','vol','dividends','splits'))
        # sem informação de data, baixa
        ...
        try:
            ...
            print("[",codigo,"] Código não disponível")
        # imprime informação
...
def atualizar_cotacoes_e_descricoes_b3_acoes_eod(codigos_da_carteira):
    ...
    path = 'D:/marin/Documents/dados/B3/IBRA/B3-IBRA.daily/'
    # path = 'D:/marin/Documents/dados/b3/FII/B3-FII.daily/'
    os.makedirs(path, exist_ok=True)
    for codigo in codigos_da_carteira:
        codigo = codigo.replace('.sa','')
        filename_csv = path+codigo+'.sa.csv'
        print("[",codigo,"] Buscando cotações...")
